...
def analyze(df):
    ...
    top_404_ip = df_404["ip"].value_counts().head(10) if not df_404.empty else pd.Series()
    ...
    # Donut chart: Mã lỗi
    wedges, texts, autotexts = axes[1].pie(status_counts.values, labels=status_counts.index, 
    ...
    axes[1].set_title("Tỷ lệ mã lỗi", fontsize=12, fontweight="bold")

    plt.tight_layout()
    st.pyplot(fig)

    # Bảng phụ: Top IP gây lỗi 404
    if not top_404_ip.empty:
        st.subheader("🔍 Top IP gây lỗi 404")
        st.table(top_404_ip.reset_index().rename(columns={"index": "IP", "ip": "Số lỗi 404"}))
    else:
        st.info("Không có lỗi 404 trong log")